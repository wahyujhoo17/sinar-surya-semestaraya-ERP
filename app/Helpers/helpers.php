...
if (!function_exists('accounting')) {
    /**
     * Get accounting configuration from database
     * Menggantikan config('accounting.*') dengan database-driven configuration
     *
     * @param string $key Format: 'transaction_type.account_key' (e.g., 'pembayaran_hutang.hutang_usaha')
     * @return int|null Account ID
     */
    function accounting($key)
    {
        return \App\Models\AccountingConfiguration::get($key);
    }
}
