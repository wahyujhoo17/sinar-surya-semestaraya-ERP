<x-app-layout :breadcrumbs="[['label' => 'Daily Aktivitas', 'url' => route('daily-aktivitas.index')], ['label' => 'Tambah Aktivitas']]" :currentPage="'Tambah Aktivitas'">
    ...
    <div class="max-w-full mx-auto py-8 sm:px-6 lg:px-8">
            ...
            <div class="flex flex-col md:flex-row md:items-center md:justify-between">
                <div class="flex-1 min-w-0">
                    ...
                    <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">
                        Buat aktivitas baru untuk membantu mengelola jadwal harian Anda.
                    </p>
                </div>
        ...
        <div
            ...
            <form action="{{ route('daily-aktivitas.store') }}" method="POST" enctype="multipart/form-data"
                x-data="aktivitasForm()" @submit="loading = true">
                @csrf
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        <div class="lg:col-span-2">
                            ...
                            <textarea id="deskripsi" name="deskripsi" rows="3"
                                class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-primary-500 focus:border-primary-500"
                                placeholder="Masukkan deskripsi aktivitas">{{ old('deskripsi') }}</textarea>
                            @error('deskripsi')
                                <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <select id="tipe_aktivitas" name="tipe_aktivitas" required
                                ...
                                @foreach (\App\Models\DailyAktivitas::getTipeAktivitasList() as $key => $label)
                                    <option value="{{ $key }}"
                                        {{ old('tipe_aktivitas') == $key ? 'selected' : '' }}>{{ $label }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select id="prioritas" name="prioritas" required
                                ...
                                @foreach (\App\Models\DailyAktivitas::getPrioritasList() as $key => $label)
                                    <option value="{{ $key }}"
                                        {{ old('prioritas') == $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                            @error('prioritas')
                                <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        ...
                        <div>
                            ...
                            <select id="status" name="status" required
                                ...
                                @foreach (\App\Models\DailyAktivitas::getStatusList() as $key => $label)
                                    <option value="{{ $key }}"
                                        {{ old('status', 'pending') == $key ? 'selected' : '' }}>{{ $label }}
                                    </option>
                                @endforeach
                            </select>
                            @error('status')
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <input type="number" id="estimasi_durasi" name="estimasi_durasi"
                                    value="{{ old('estimasi_durasi') }}" step="0.5" min="0"
                                    max="999.99"
                                    class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-primary-500 focus:border-primary-500"
                        ...
                        <div class="space-y-6">
                            <div>
                                ...
                                <textarea id="peserta" name="peserta" rows="2"
                                    class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-primary-500 focus:border-primary-500"
                                    placeholder="Masukkan nama peserta, email, atau daftar yang terlibat">{{ old('peserta') }}</textarea>
                                @error('peserta')
                                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <textarea id="catatan" name="catatan" rows="3"
                                    class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-primary-500 focus:border-primary-500"
                                    placeholder="Masukkan catatan atau informasi tambahan">{{ old('catatan') }}</textarea>
                                @error('catatan')
                                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                                    Upload file pendukung (PDF, DOC, Image, Excel, PowerPoint). Maksimal 10MB per file.
                                </p>
                                @error('attachments')
                                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                                @enderror
                ...
                <div
                    ...
                    <button type="submit" :disabled="loading"
                        ...
                        <span x-text="loading ? 'Menyimpan...' : 'Simpan Aktivitas'"></span>
                    </button>
                </div>
            </form>
        </div>
    ...
    <script>
        function aktivitasForm() {
            return {
                ...
                init() {
                    // Auto-set reminder time 30 minutes before start time
                    ...
                    tanggalMulaiInput.addEventListener('change', function() {
                        if (this.value && !reminderInput.value) {
                            ...
                            const reminderTime = new Date(startTime.getTime() - 30 * 60000); // 30 minutes before
