        ...
        <nav class="flex-1 py-4 px-2 space-y-1" x-data="{
                        ...
                        @if (auth()->user()->hasPermission('pipeline_penjualan.view'))
                            <a href="{{ route('crm.pipeline.index') }}"
                                class="{{ request()->is('crm/pipeline') || request()->is('crm/pipeline/*') ? 'text-blue-700 dark:text-blue-400 font-semibold' : 'text-gray-600 hover:text-gray-900 dark:text-gray-300 dark:hover:text-white' }} group flex items-center px-3 py-2 text-sm rounded-md transition-colors"><span>Pipeline
                                    Penjualan</span></a>
                        @endif
                        @if (auth()->user()->hasPermission('laporan_crm_view'))
                        ...
                        @if (auth()->user()->hasPermission('kas_dan_bank.view'))
                            <a href="/keuangan/pengembalian-dana"
                                class="{{ request()->is('keuangan/pengembalian-dana') || request()->is('keuangan/pengembalian-dana/*') ? 'text-blue-700 dark:text-blue-400 font-semibold' : 'text-gray-600 hover:text-gray-900 dark:text-gray-300 dark:hover:text-white' }} group flex items-center px-3 py-2 text-sm rounded-md transition-colors"><span>Pengembalian
                                    Dana</span></a>
                        @endif
                        @if (auth()->user()->hasPermission('uang_muka_penjualan.view') || auth()->user()->hasPermission('kas_dan_bank.view'))
                            <a href="{{ route('keuangan.uang-muka-penjualan.index') }}"
                                class="{{ request()->is('keuangan/uang-muka-penjualan') || request()->is('keuangan/uang-muka-penjualan/*') ? 'text-blue-700 dark:text-blue-400 font-semibold' : 'text-gray-600 hover:text-gray-900 dark:text-gray-300 dark:hover:text-white' }} group flex items-center px-3 py-2 text-sm rounded-md transition-colors"><span>Uang
                                    Muka Penjualan</span></a>
                        @endif
                    </div>
                </div>
            @endif
            ...
            @if (auth()->user()->karyawan)
                <a href="{{ route('hr.slip-gaji.index') }}"
                    ...
                    :class="{ 'justify-center': sidebarCollapsed }"
                    x-tooltip="sidebarCollapsed ? 'Slip Gaji' : ''">
                    <div class="flex-shrink-0 h-9 w-9 flex items-center justify-center rounded-lg bg-white dark:bg-gray-700 text-gray-500 dark:text-gray-300 shadow-sm sidebar-card-icon"
                        :class="{ 'mr-0': sidebarCollapsed, 'mr-3': !sidebarCollapsed }">
                    ...
                    <span x-show="!sidebarCollapsed" class="transition-opacity duration-300">Slip Gaji</span>
                </a>
            @endif
