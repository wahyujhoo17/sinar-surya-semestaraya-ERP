@forelse ($kategoris as $kategori)
    <tr class="hover:bg-gray-50/80 dark:hover:bg-gray-700/50 transition-all duration-150">
        ...
        <td class="px-5 py-4 whitespace-nowrap text-right text-sm font-medium">
            @if (auth()->user()->hasPermission('kategori_produk.edit') || auth()->user()->hasPermission('kategori_produk.delete'))
                <div x-data="{ open: false }" class="relative inline-block text-left">
                    ...
                    <div x-show="open" x-transition
                        ...
                        @if (auth()->user()->hasPermission('kategori_produk.edit'))
                ...
                @if (auth()->user()->hasPermission('kategori_produk.delete'))
                    <form id="delete-form-{{ $kategori->id }}"
                        action="{{ route('master.kategori-produk.destroy', $kategori->id) }}" method="POST"
                        class="hidden">
                        @csrf
                        @method('DELETE')
                    </form>
