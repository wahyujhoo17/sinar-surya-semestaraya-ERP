        ...
        <td class="px-5 py-4 whitespace-nowrap text-right text-sm font-medium">
            <div x-data="{ open: false }" class="relative inline-block text-left">
                ...
                <div x-show="open" x-transition:enter="transition ease-out duration-100"
                    ...
                    <div class="py-1" role="none">
                        ...
                        @endif
                        @if (auth()->user()->hasPermission('pelanggan.edit'))
            ...
            @if (auth()->user()->hasPermission('pelanggan.delete'))
                <form id="delete-form-{{ $customer->id }}"
                    action="{{ route('master.pelanggan.destroy', $customer->id) }}" method="POST" class="hidden">
                    @csrf
                    @method('DELETE')
                </form>
            @endif
    ...
    <tr>
        ...
        <td id="empty-row-td" colspan="7" class="px-5 py-12 text-center"> {{-- Example: 5 default visible + 2 = 7 --}}
            <div class="flex flex-col items-center justify-center">
                ...
                @else
                    <div
                        class="inline-flex items-center px-4 py-2 bg-gray-300 text-gray-600 text-sm font-medium rounded-lg shadow-sm opacity-60 cursor-not-allowed">
                        <svg class="w-4 h-4 mr-2" xmlns="http://www.w3.org/2000/svg" fill="none"
                            viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round"
