@forelse ($gudangs as $gudang)
    <tr class="hover:bg-gray-50/80 dark:hover:bg-gray-700/50 transition-all duration-150">
        <td class="px-3 py-4">
            <input type="checkbox" name="gudang_ids[]" value="{{ $gudang->id }}"
                @click="updateSelectedGudangs($event, '{{ $gudang->id }}')"
        ...
        <td class="px-5 py-4 text-sm text-gray-700 dark:text-gray-400">
            {{ $gudang->jenis ? ucfirst($gudang->jenis) : '-' }}
        </td>
        <td class="px-5 py-4 text-sm text-gray-700 dark:text-gray-400">
            {{ $gudang->alamat ? \Illuminate\Support\Str::limit($gudang->alamat, 40) : '-' }}
        </td>
        ...
        <td class="px-5 py-4 text-center text-sm text-gray-700 dark:text-gray-300">
            {{ $gudang->stok_count ?? 0 }}
        </td>
        <td class="px-5 py-4 whitespace-nowrap text-right text-sm font-medium">
            <div x-data="{ open: false }" class="relative inline-block text-left">
                ...
                <div x-show="open" x-transition
                    ...
                    <div class="py-1" role="none">
                        <a href="#" @click.prevent="openEditGudangModal('{{ $gudang->id }}')"
                    ...
                    <div class="py-1">
                        <button type="button"
                            @click="open = false; confirmDelete('Apakah Anda yakin ingin menghapus gudang {{ $gudang->nama }}?', () => document.getElementById('delete-form-{{ $gudang->id }}').submit())"
            ...
            <form id="delete-form-{{ $gudang->id }}" action="{{ route('master.gudang.destroy', $gudang->id) }}"
    ...
    <tr>
        <td colspan="8" class="px-5 py-12 text-center">
            <div class="flex flex-col items-center justify-center">
                ...
                <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-1">Tidak ada gudang ditemukan</h3>
                <p class="text-sm text-gray-500 dark:text-gray-400 text-center max-w-md mb-6">
                    @if (request()->has('search'))
                        Tidak ada gudang yang sesuai dengan pencarian Anda.
                        <a href="{{ route('master.gudang.index') }}"
                            class="text-primary-600 dark:text-primary-400 hover:underline">Reset filter</a>.
                    @else
                        Belum ada gudang yang ditambahkan. Mulai dengan menambahkan gudang baru.
                    @endif
                </p>
                <a href="#"
                    @click.prevent="window.dispatchEvent(new CustomEvent('open-gudang-modal', {detail: {}}))"
