<x-app-layout :breadcrumbs="[
    ...
    <div class="py-8 max-w-full mx-auto sm:px-6 lg:px-8">
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6 border border-gray-200 dark:border-gray-700 mb-6">
            ...
            <form action="{{ route('penjualan.delivery-order.update', $deliveryOrder->id) }}" method="POST" id="formDeliveryOrder">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                    <div>
                        ...
                        <input type="text" name="nomor" id="nomor" value="{{ old('nomor', $deliveryOrder->nomor) }}" readonly
                    ...
                    <div>
                        ...
                        <input type="date" name="tanggal" id="tanggal" value="{{ old('tanggal', $deliveryOrder->tanggal) }}" required
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
                    </div>
                    <div wire:ignore>
                    ...
                    <div wire:ignore>
                        ...
                        <select name="gudang_id" id="gudang_id" required class="w-full">
                            ...
                            @foreach ($gudangs as $gudang)
                                <option value="{{ $gudang->id }}" {{ old('gudang_id', $deliveryOrder->gudang_id) == $gudang->id ? 'selected' : '' }}>
                                    {{ $gudang->nama }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="alamat_pengiriman" id="alamat_pengiriman" rows="3" required
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">{{ old('alamat_pengiriman', $deliveryOrder->alamat_pengiriman) }}</textarea>
                    </div>
                    <div class="md:col-span-2">
                        <label for="catatan" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Catatan</label>
                        <textarea name="catatan" id="catatan" rows="2"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">{{ old('catatan', $deliveryOrder->catatan) }}</textarea>
                    </div>
                </div>
    ...
    @push('scripts')
        ...
        <script>
            ...
            var existingItems = {!! json_encode($deliveryOrder->details->map(function($item, $index) {
                $is_bundle = $item->is_bundle_item;
                return [
                    'id' => $index + 1,
            ...
            function addRow(data = {}) {
                ...
                let html = `
                    <tr id="row_${id}" class="product-row bg-white dark:bg-gray-900">
                        <td class="px-4 py-3">
                            <input type="hidden" name="delivery_order_detail_id[]" value="${data.delivery_order_detail_id || ''}">
            ...
            $(document).ready(function() {
                ...
                $('#formDeliveryOrder').submit(function(e) {
                    ...
                    $('#submitBtn').html('<i class="fas fa-spinner fa-spin mr-2"></i>Menyimpan...');
                                ...
                                // Init with existing rows
                if (typeof existingItems !== 'undefined' && existingItems && existingItems.length > 0) {
                    $('#productTableBody').empty(); // Clear table first to avoid duplicate init
                    existingItems.forEach(item => {
                        addRow(item);
                    });
                    setTimeout(fetchStockInfo, 500); // Fetch stock for initial items
                } else {
                    addRow();
                }
            });
