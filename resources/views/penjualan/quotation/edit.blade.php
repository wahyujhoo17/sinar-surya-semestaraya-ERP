                                                ...
                                                <div class="md:col-span-3">
                                                    ...
                                                    <div class="mt-1 relative rounded-md shadow-sm">
                                                        ...
                                                        <input type="text" inputmode="numeric"
                                                            :value="item.harga > 0 ? Number(item.harga).toLocaleString('id-ID') : ''"
                    ...
                    <div
                        ...
                        <div class="flex justify-between items-center mt-2">
                            ...
                            <div class="flex items-center space-x-2">
                                ...
                                <input type="text" inputmode="numeric" :value="$data.ongkosKirim > 0 ? Number($data.ongkosKirim).toLocaleString('id-ID') : ''"
                ...
                return {
                    ...
                    init() {
                        ...
                        if (Array.isArray(quotationItemsData) && quotationItemsData.length > 0) {
                            // Process items with proper bundle handling
                            const existingItems = [];
                            const bundleGroups = new Map();

                            // First pass: collect all items and identify bundle structure
                            quotationItemsData.forEach((item, idx) => {
                                const harga = parseFloat(item.harga) || 0;
                                const qty = parseFloat(item.kuantitas || item.quantity) || 1;
                                ...
                                const processedItem = {
                                    ...
                                    // is_bundle: akan dibuat secara terpisah sebagai header
                                    is_bundle: false,
                                    is_bundle_item: item.is_bundle_item || (item.item_type === 'bundle_item'),
                                    bundle_name: item.bundle?.nama || item.bundle_name || '',
                                    bundle_code: item.bundle?.kode || item.bundle_code || '',
                                    bundle_gambar: item.bundle?.gambar || ''
                                };
                            ...
                            // Group bundle items by bundle_id and create headers
                            bundleItems.forEach(item => {
                                ...
                                if (!bundleGroups.has(item.bundle_id)) {
                                    bundleGroups.set(item.bundle_id, {
                                        ...
                                        bundle_gambar: item.bundle_gambar,
                                        items: []
                                    });
                                }
                            ...
                            // Create bundle headers and add to items array
                            bundleGroups.forEach((group, bundleId) => {
                                // Calculate bundle quantity (assume all items in bundle have same quantity ratio)
                                const bundleQuantity = group.items.length > 0 ?
                                    Math.max(1, Math.min(...group.items.map(item => item.kuantitas))) : 1;

                                // Get bundle data to get bundle discount
                                ...
                                // console.log('Created bundle header:', bundleHeader);
                        ...
                        // console.log('Quotation Items Data:', quotationItemsData);
                        // console.log('Processed Items:', this.items);
                        // console.log('Items with bundles:', this.items.filter(item => item.bundle_id || item.is_bundle));
                        // console.log('Items with is_bundle true:', this.items.filter(item => item.is_bundle === true));
                        // console.log('Bundle headers specifically:', this.items.filter(item => item.is_bundle === true).map(
                        //     item => ({
                        //         id: item.id,
                        //         bundle_name: item.bundle_name,
                        //         bundle_code: item.bundle_code,
                    ...
                    removeItem(index) {
                        ...
                        if (item.is_bundle && item.bundle_id) {
                            // console.log('Removing bundle header and its items for bundle_id:', item.bundle_id);
                            ...
                            for (let i = this.items.length - 1; i >= 0; i--) {
                                ...
                                if (currentItem.bundle_id === item.bundle_id) {
                                    ...
                                    // console.log('Found item to remove:', currentItem);
                                }
                            }
                    ...
                    async addBundleToItems(bundle) {
                        try {
                            ...
                            // console.log('Fetching bundle data from URL:', url);
                    ...
                    async processBundleData(bundleData, bundleItems) {
                        try {
                            ...
                            const bundleMainItem = {
                                ...
                                diskon_nominal: bundleDiskonNominal,
                                subtotal: bundleSubtotalAfter,
                                is_bundle: true,
                                is_bundle_item: false,
                            ...
                            // Add individual bundle items as child items with bundle discount applied
                            ...
                            // console.log('Bundle added - total items now:', this.items.length);
