...
<body>
            ...
            @foreach ($quotation->details as $index => $detail)
                @if ($detail->bundle_id && !in_array($detail->bundle_id, $processedBundles))
                    @php
                        ...
                        // (bundle headers are not saved to DB, so we derive qty from the child items)
                        $firstChild = $bundleItems->first();
                        $bundleDef = $firstChild ? $firstChild->bundle : null;
                        $bundleQty = 1;
                        $bundleUnitPrice = 0;
                        if ($bundleDef) {
                            $defItem = $bundleDef->items->where('produk_id', $firstChild->produk_id)->first();
                            if ($defItem && $defItem->quantity > 0) {
                                $bundleQty = max(1, round((float) $firstChild->quantity / (float) $defItem->quantity));
                            }
                    ...
                    <tr class="bundle-header">
                        ...
                        <td class="text-right">Rp {{ number_format($bundleUnitPrice, 0, ',', '.') }}</td>
    ...
    @php
        // Recompute subtotal using harga_bundle for bundles (avoids per-item discount rounding diff)
        $pdfSubtotal = 0;
        $processedBundleIds = [];
        foreach ($quotation->details as $_d) {
            if ($_d->bundle_id && $_d->is_bundle_item && !in_array($_d->bundle_id, $processedBundleIds)) {
                $processedBundleIds[] = $_d->bundle_id;
                $_bItems = $quotation->details->where('bundle_id', $_d->bundle_id)->where('is_bundle_item', true);
                $_bDef = $_d->bundle;
                if ($_bDef) {
    ...
    <table class="summary-table">
        <tr>
            <td>Subtotal</td>
            <td class="text-right">Rp {{ number_format($pdfSubtotal, 0, ',', '.') }}</td>
        </tr>
        @if ($pdfDiskonNominal > 0)
            <tr>
                <td>Diskon ({{ number_format($quotation->diskon_persen, 1) }}%)</td>
                <td class="text-right">Rp {{ number_format($pdfDiskonNominal, 0, ',', '.') }}</td>
            </tr>
        @endif
        @if ($quotation->ppn > 0)
            <tr>
                <td>PPN ({{ $quotation->ppn }}%)</td>
                <td class="text-right">Rp {{ number_format($pdfPpnValue, 0, ',', '.') }}</td>
            </tr>
        @endif
        ...
        <tr class="total-row">
            ...
            <td class="text-right"><strong>Rp {{ number_format($pdfTotal, 0, ',', '.') }}</strong></td>
        </tr>
    </table>
