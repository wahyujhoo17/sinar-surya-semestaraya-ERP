        ...
        @forelse($returPembelian as $retur)
            <div class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg p-4 shadow-sm">
                ...
                <div class="flex items-center justify-between mb-3">
                    <div class="font-medium text-gray-900 dark:text-white">{{ $retur->nomor }}</div>
                    <div class="flex items-center space-x-1">
                        @if (auth()->user()->hasPermission('retur_pembelian.view'))
                            <a href="{{ route('pembelian.retur-pembelian.show', $retur) }}"
                                class="inline-flex items-center justify-center h-7 w-7 rounded-lg hover:bg-blue-100 text-gray-700 dark:text-white dark:bg-blue-900/20 dark:hover:bg-blue-900/30 transition-colors border border-dashed border-blue-300"
                                title="Detail">
                        ...
                        @endif
                        @if (auth()->user()->hasPermission('retur_pembelian.edit') && $retur->status === 'draft')
                            <a href="{{ route('pembelian.retur-pembelian.edit', $retur) }}"
                                class="inline-flex items-center justify-center h-7 w-7 rounded-lg hover:bg-yellow-100 text-gray-700 dark:text-white dark:bg-yellow-900/20 dark:hover:bg-yellow-900/30 transition-colors border border-dashed border-yellow-300"
                                title="Edit">
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"
                                    class="w-3.5 h-3.5">
                                    <path
                        ...
                        @endif
                        @if (auth()->user()->hasPermission('retur_pembelian.delete') && $retur->status === 'draft')
                            <form action="{{ route('pembelian.retur-pembelian.destroy', $retur) }}" method="POST"
                                ...
                                <button type="submit"
                                    ...
                                    class="inline-flex items-center justify-center h-7 w-7 rounded-lg hover:bg-red-100 text-gray-700 dark:text-white dark:bg-red-900/20 dark:hover:bg-red-900/30 transition-colors border border-dashed border-red-300"
                                    title="Hapus">
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"
                                        class="w-3.5 h-3.5">
                                        <path fill-rule="evenodd"
                ...
                <div class="space-y-2 text-sm">
                    <div class="flex justify-between">
                        ...
                        <span
                            class="text-gray-900 dark:text-white">{{ \Carbon\Carbon::parse($retur->tanggal)->format('d/m/Y') }}</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-gray-500 dark:text-gray-400">Supplier:</span>
                        <span
                            class="text-gray-900 dark:text-white text-right">{{ optional($retur->supplier)->nama ?? 'Tidak Tersedia' }}</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-gray-500 dark:text-gray-400">PO:</span>
                        <span
                            class="text-gray-900 dark:text-white">{{ optional($retur->purchaseOrder)->nomor ?? 'Tidak Tersedia' }}</span>
                    </div>
                    <div class="flex justify-between">
        ...
        @empty
            <div class="text-center py-8">
                <div class="flex flex-col items-center justify-center">
                    ...
                    <span class="text-gray-500 dark:text-gray-400">Belum ada data Retur Pembelian.</span>
        ...
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            <thead>
                <tr class="bg-gray-50 dark:bg-gray-700/50">
                    <th
                        class="px-3 sm:px-5 py-3.5 text-left text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase tracking-wider min-w-[120px]">
                        <a href="{{ request()->fullUrlWithQuery(['sort_by' => 'nomor', 'sort_direction' => request('sort_by') == 'nomor' && request('sort_direction') == 'asc' ? 'desc' : 'asc']) }}"
                            ...
                            <span class="truncate">No. Retur</span>
                    ...
                    <th
                        class="px-3 sm:px-5 py-3.5 text-left text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase tracking-wider min-w-[100px]">
                    ...
                    <th
                        class="px-3 sm:px-5 py-3.5 text-left text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase tracking-wider min-w-[150px]">
                    ...
                    <th
                        class="px-3 sm:px-5 py-3.5 text-left text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase tracking-wider min-w-[120px]">
                        <span class="truncate">PO</span>
                    </th>
                    <th
                        class="px-3 sm:px-5 py-3.5 text-left text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase tracking-wider min-w-[140px]">
                        <span class="truncate">Tipe Retur</span>
                    </th>
                    <th
                        class="px-3 sm:px-5 py-3.5 text-left text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase tracking-wider min-w-[120px]">
                    ...
                    <th
                        class="px-3 sm:px-5 py-3.5 text-right text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase tracking-wider min-w-[100px] sticky right-0 bg-gray-50 dark:bg-gray-700/50">
            ...
            <tbody class="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-800">
                @forelse($returPembelian as $retur)
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/60">
                        ...
                        <td class="px-3 sm:px-5 py-4 whitespace-nowrap">
                            <div class="text-sm text-gray-700 dark:text-gray-300 truncate max-w-[150px]"
                                title="{{ optional($retur->supplier)->nama ?? 'Tidak Tersedia' }}">
                                {{ optional($retur->supplier)->nama ?? 'Tidak Tersedia' }}
                            </div>
                        </td>
                        <td class="px-3 sm:px-5 py-4 whitespace-nowrap">
                            <div class="text-sm text-gray-700 dark:text-gray-300 truncate max-w-[120px]"
                                title="{{ optional($retur->purchaseOrder)->nomor ?? 'Tidak Tersedia' }}">
                        ...
                        <td class="px-3 sm:px-5 py-4 whitespace-nowrap">
                            ...
                            <span
                                class="inline-flex px-2.5 py-1 rounded-full text-xs font-medium bg-{{ $statusColor }}-100 text-{{ $statusColor }}-800 dark:bg-{{ $statusColor }}-900/30 dark:text-{{ $statusColor }}-400 border border-{{ $statusColor }}-200 dark:border-{{ $statusColor }}-700">
                                <span
                                    class="w-1.5 h-1.5 rounded-full bg-{{ $statusColor }}-500 mr-1.5 my-auto"></span>
                                <span class="truncate max-w-[80px]">{{ $statusText }}</span>
                            </span>
                        </td>
                        <td
                            class="px-3 sm:px-5 py-4 whitespace-nowrap text-right sticky right-0 bg-white dark:bg-gray-900">
                            <div class="flex justify-end space-x-1">
                                <!-- Show button - visible with view permission -->
                                @if (auth()->user()->hasPermission('retur_pembelian.view'))
                                    <a href="{{ route('pembelian.retur-pembelian.show', $retur) }}"
                                        class="inline-flex items-center justify-center h-7 w-7 rounded-lg hover:bg-blue-100 text-gray-700 dark:text-white dark:bg-blue-900/20 dark:hover:bg-blue-900/30 transition-colors border border-dashed border-blue-300"
                                        title="Detail">
                                ...
                                <!-- Edit button - only visible for draft status with edit permission -->
                                @if (auth()->user()->hasPermission('retur_pembelian.edit') && $retur->status === 'draft')
                                    <a href="{{ route('pembelian.retur-pembelian.edit', $retur) }}"
                                        class="inline-flex items-center justify-center h-7 w-7 rounded-lg hover:bg-yellow-100 text-gray-700 dark:text-white dark:bg-yellow-900/20 dark:hover:bg-yellow-900/30 transition-colors border border-dashed border-yellow-300"
                                        title="Edit">
                                ...
                                <!-- Delete button - only visible for draft status with delete permission -->
                                @if (auth()->user()->hasPermission('retur_pembelian.delete') && $retur->status === 'draft')
                                    <form action="{{ route('pembelian.retur-pembelian.destroy', $retur) }}"
                ...
                @empty
                    <tr>
                        <td colspan="7"
                            class="px-3 sm:px-5 py-8 text-center text-sm text-gray-500 dark:text-gray-400">
                            <div class="flex flex-col items-center justify-center">
