...
<x-app-layout>
    <div class="max-w-full mx-auto py-6 sm:px-6 lg:px-8">
            ...
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                    <div class="flex items-center">
                        ...
                        <div>
                            <h1 class="text-2xl md:text-3xl font-bold text-gray-900 dark:text-white">Purchase Order
                            </h1>
                            <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
                                Kelola purchase order untuk pengadaan barang dari supplier.
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <div class="mt-6 grid grid-cols-1 md:grid-cols-5 gap-4">
                @foreach ($validStatuses as $status)
                    <div
                        ...
                        <div
                            class="absolute -right-4 -top-4 h-16 w-16 rounded-full bg-{{ poStatusColor($status) }}-50 dark:bg-{{ poStatusColor($status) }}-900/20 opacity-70">
                        </div>
                        <div class="p-4">
                            ...
                            <div
                                class="absolute bottom-0 left-0 right-0 h-1 bg-{{ poStatusColor($status) }}-500 dark:bg-{{ poStatusColor($status) }}-600 opacity-80">
                            </div>
                        </div>
                    </div>
                @endforeach
        ...
        {{-- Main Content --}}
        <div x-data="purchaseOrderTableManager()"
                        ...
                        @foreach ($validStatuses as $status)
                            <button type="button"
                                ...
                                <span class="flex items-center gap-2">
                                    <span class="w-2 h-2 rounded-full bg-{{ poStatusColor($status) }}-500"></span>
                                    {{ ucfirst($status) }}
                                </span>
                            </button>
                        @endforeach
                ...
                <form @submit.prevent="applyFilters" class="flex flex-wrap gap-3 items-end">
                    ...
                    <div class="w-full sm:w-auto space-y-1">
                        <label for="payment-status"
                            class="block text-xs font-medium text-gray-700 dark:text-gray-300">Pembayaran</label>
                        <select id="payment-status" x-model="paymentStatus"
                            class="pl-3 pr-8 py-2 border border-gray-200 dark:border-gray-700 rounded-lg text-sm bg-white dark:bg-gray-800 text-gray-800 dark:text-white focus:ring-primary-500 focus:border-primary-500 transition-colors">
            ...
            <div class="relative px-1 sm:px-3 pb-6">
                ...
                {{-- Tambahkan tableHtml di sini --}}
                <div x-html="tableHtml"></div>

                <div id="po-pagination-container" x-html="paginationHtml" class="mt-6 px-5">
                    @if (!request()->ajax())
                        {{ $purchaseOrders->appends(request()->except('page'))->links('vendor.pagination.tailwind-custom') }}
                    @endif
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        function purchaseOrderTableManager() {
            return {
                ...
                fetchTable() {
                    this.loading = true;
                    const url = `{{ route('pembelian.purchasing-order.index') }}?${this.buildQueryString()}`;
                    fetch(url, {
                            headers: {
                ...
                attachPaginationListener() {
                    this.$nextTick(() => {
                        document.querySelectorAll('#po-pagination-container a').forEach(link => {
                            link.addEventListener('click', e => {
                                ...
                                if (page) {
                                    ...
                                    this.loading = true;
                                    fetch(`{{ route('pembelian.purchasing-order.index') }}?${params}`, {
                                            headers: {
                                                'X-Requested-With': 'XMLHttpRequest'
                                            }
                                        })
