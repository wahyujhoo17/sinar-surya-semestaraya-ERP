                ...
                <form action="{{ route('crm.aktivitas.update', $aktivitas->id) }}" method="POST"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="col-span-1">
                            ...
                            <select id="entity_type" name="entity_type"
                                ...
                                <option value="prospek"
                                    {{ old('entity_type', $aktivitas->prospek_id ? 'prospek' : ($aktivitas->customer_id ? 'customer' : '')) == 'prospek' ? 'selected' : '' }}>
                                    Prospek / Lead
                                </option>
                                <option value="customer"
                                    {{ old('entity_type', $aktivitas->prospek_id ? 'prospek' : ($aktivitas->customer_id ? 'customer' : '')) == 'customer' ? 'selected' : '' }}>
                                    Customer / Konsumen
                                </option>
                            </select>
                            @error('entity_type')
                        ...
                        <div class="col-span-1" id="prospek-field"
                            style="display: {{ old('entity_type', $aktivitas->prospek_id ? 'prospek' : ($aktivitas->customer_id ? 'customer' : '')) == 'prospek' ? 'block' : 'none' }}">
                            ...
                            <select id="prospek_id" name="prospek_id"
                                ...
                                @foreach ($prospekList as $prospek)
                                    <option value="{{ $prospek->id }}"
                                        {{ old('prospek_id', $aktivitas->prospek_id) == $prospek->id ? 'selected' : '' }}>
                        ...
                        <div class="col-span-1" id="customer-field"
                            style="display: {{ old('entity_type', $aktivitas->prospek_id ? 'prospek' : ($aktivitas->customer_id ? 'customer' : '')) == 'customer' ? 'block' : 'none' }}">
                            ...
                            <select id="customer_id" name="customer_id"
                                ...
                                @foreach ($customerList as $customer)
                                    <option value="{{ $customer->id }}"
                                        {{ old('customer_id', $aktivitas->customer_id) == $customer->id ? 'selected' : '' }}
                        ...
                        <div class="col-span-1">
                            ...
                            <select id="tipe" name="tipe"
                                ...
                                @foreach ($tipeList as $key => $label)
                                    <option value="{{ $key }}"
                                        {{ old('tipe', $aktivitas->tipe) == $key ? 'selected' : '' }}>
                                        {{ $label }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-span-1">
                            ...
                            <div class="relative">
                                <input type="datetime-local" id="tanggal" name="tanggal"
                                    value="{{ old('tanggal', $aktivitas->tanggal ? $aktivitas->tanggal->format('Y-m-d\TH:i') : now()->format('Y-m-d\TH:i')) }}"
                        ...
                        <div class="col-span-full">
                            <label for="deskripsi"
                                class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1 flex items-center">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1.5 text-blue-500"
                                    fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            <div class="relative mt-1">
                                <textarea id="deskripsi" name="deskripsi" rows="3"
                                    ...
                                    placeholder="Detail aktivitas...">{{ old('deskripsi', $aktivitas->deskripsi) }}</textarea>
                            </div>
                            @error('deskripsi')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="col-span-full">
                            ...
                            <textarea id="hasil" name="hasil" rows="3"
                                ...
                                placeholder="Hasil dari aktivitas...">{{ old('hasil', $aktivitas->hasil) }}</textarea>
                            @error('hasil')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                                ...
                                <div class="flex items-start">
                                    <div class="flex items-center h-5">
                                        <input id="perlu_followup" name="perlu_followup" type="checkbox"
                                            ...
                                            {{ old('perlu_followup', $aktivitas->perlu_followup) ? 'checked' : '' }}
                        ...
                        <!-- Follow-up Fields (initially hidden if not checked) -->
                        <div id="followup-fields"
                            class="col-span-full grid grid-cols-1 md:grid-cols-2 gap-6 {{ old('perlu_followup', $aktivitas->perlu_followup) ? '' : 'hidden' }}">
                            <!-- Tanggal Follow-up -->
                            <div>
                                ...
                                <div class="relative">
                                    <input type="datetime-local" id="tanggal_followup" name="tanggal_followup"
                                        value="{{ old('tanggal_followup', $aktivitas->tanggal_followup ? $aktivitas->tanggal_followup->format('Y-m-d\TH:i') : now()->addDays(3)->format('Y-m-d\TH:i')) }}"
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    <select id="status_followup" name="status_followup"
                                        class="block w-full pl-10 rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-primary-500 focus:ring-primary-500 @error('status_followup') border-red-500 @enderror">
                                        @foreach ($statusList as $key => $label)
                                            <option value="{{ $key }}"
                                                {{ old('status_followup', $aktivitas->status_followup ?? \App\Models\ProspekAktivitas::STATUS_MENUNGGU) == $key ? 'selected' : '' }}>
                                                {{ $label }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-span-full">
                                ...
                                <textarea id="catatan_followup" name="catatan_followup" rows="3"
                                    ...
                                    placeholder="Catatan untuk follow-up...">{{ old('catatan_followup', $aktivitas->catatan_followup) }}</textarea>
