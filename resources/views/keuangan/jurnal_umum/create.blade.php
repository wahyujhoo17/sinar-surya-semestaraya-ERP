<x-app-layout :breadcrumbs="$breadcrumbs" :currentPage="$currentPage">
    <div class="max-w-full mx-auto py-8 sm:px-6 lg:px-8">
        ...
        <div class="mb-8 flex flex-col md:flex-row md:items-center md:justify-between">
            <div>
                <h1 class="text-2xl md:text-3xl font-bold text-gray-900 dark:text-white">Tambah Jurnal Baru</h1>
                <p class="mt-2 text-gray-600 dark:text-gray-400">
                    Tambahkan entri jurnal umum baru untuk PT Sinar Surya Semestaraya
                </p>
            </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md overflow-hidden">
            <form action="{{ route('keuangan.jurnal-umum.store') }}" method="POST" id="jurnalUmumForm">
                @csrf
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            ...
                            <div>
                                ...
                                <input type="text" name="no_referensi" id="no_referensi"
                                    value="{{ old('no_referensi', 'JU-' . date('Ymd') . '-' . rand(1000, 9999)) }}"
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white"
                                    required>
                            </div>
                                ...
                                <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-600">
                                    <tr>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <select name="jurnal_items[0][akun_id]"
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <input type="number" name="jurnal_items[0][debit]" step="0.01"
                                                min="0"
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <input type="number" name="jurnal_items[0][kredit]" step="0.01"
                                                min="0"
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-center">
                                            <button type="button"
                                                class="text-red-500 hover:text-red-700 disabled:opacity-50 disabled:cursor-not-allowed deleteRow"
                                                disabled>
                                    ...
                                    <tr>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <select name="jurnal_items[1][akun_id]"
                                                class="block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                                                <option value="">Pilih Akun</option>
                                                @foreach ($akuns as $akun)
                                                    <option value="{{ $akun->id }}">{{ $akun->kode }} -
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <input type="number" name="jurnal_items[1][debit]" step="0.01"
                                                min="0"
                                                class="debit-input block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm text-right dark:bg-gray-700 dark:border-gray-600 dark:text-white"
                                                placeholder="0,00">
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <input type="number" name="jurnal_items[1][kredit]" step="0.01"
                                                min="0"
                                                class="kredit-input block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm text-right dark:bg-gray-700 dark:border-gray-600 dark:text-white"
                                                placeholder="0,00">
                                        </td>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            let rowCounter = 2; // Start from 2 since we already have 2 rows
            ...
            function inputHandler(e) {
                ...
                // If this is a debit input and value > 0, clear the kredit input in the same row and vice versa
                if (input.classList.contains('debit-input')) {
                    const value = parseFloat(input.value) || 0;
                    if (value > 0) {
                        row.querySelector('.kredit-input').value = '';
                    }
                } else if (input.classList.contains('kredit-input')) {
                    const value = parseFloat(input.value) || 0;
                    if (value > 0) {
            ...
            function updateTotals() {
                ...
                // Calculate totals and validate rows
                const rows = jurnalTable.querySelectorAll('tr');
                rows.forEach(row => {
                    const debitInput = row.querySelector('.debit-input');
                    const kreditInput = row.querySelector('.kredit-input');
                    const akunSelect = row.querySelector('select');

                    if (debitInput && kreditInput && akunSelect) {
                        const debit = parseFloat(debitInput.value) || 0;
                        const kredit = parseFloat(kreditInput.value) || 0;

                        // Check if akun is selected
                        if (!akunSelect.value) {
                            isValid = false;
                        }

                        // Check if exactly one of debit/kredit is filled
                ...
                // Enable/disable submit button based on balance and validations
                if (selisih < 0.01 && totalDebit > 0 && totalKredit > 0 && isValid) {
            ...
            resetButton.addEventListener('click', function() {
                if (confirm('Apakah Anda yakin ingin mereset form ini?')) {
                    document.getElementById('jurnalUmumForm').reset();
                    updateTotals();
                }
            });
