<x-app-layout :breadcrumbs="[
    ...
    ['name' => 'Edit Uang Muka', 'url' => '#'],
]" :currentPage="'Edit Uang Muka Penjualan - ' . $uangMuka->nomor">
    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                        ...
                        <div class="flex items-center space-x-4">
                            ...
                            <div>
                                <h2 class="text-xl font-semibold text-gray-900 dark:text-white">
                                    Edit Uang Muka Penjualan
                                </h2>
                                <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">
                                    Edit data uang muka: <span
                                        class="font-medium text-amber-600 dark:text-amber-400">{{ $uangMuka->nomor }}</span>
                                </p>
                            </div>
                        </div>
            ...
            <!-- Form -->
            <form action="{{ route('keuangan.uang-muka-penjualan.update', $uangMuka->id) }}" method="POST"
                x-data="uangMukaEditForm()" x-init="init()">
                @csrf
                @method('PUT')
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        ...
                                        <input type="text" name="nomor" id="nomor"
                                            value="{{ old('nomor', $uangMuka->nomor) }}"
                                    ...
                                    <div>
                                        ...
                                        <input type="date" name="tanggal" id="tanggal"
                                            value="{{ old('tanggal', $uangMuka->tanggal->format('Y-m-d')) }}"
                                            class="mt-1 block w-full border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-white"
                                            required>
                                        @error('tanggal')
                                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                        ...
                                        <select name="customer_id" id="customer_id"
                                            ...
                                            @foreach ($customers as $customer)
                                                <option value="{{ $customer->id }}"
                                                    {{ old('customer_id', $uangMuka->customer_id) == $customer->id ? 'selected' : '' }}>
                                    ...
                                    {{-- <div>
                                        ...
                                        <select name="sales_order_id" id="sales_order_id" x-model="selectedSalesOrder"
                                            ...
                                            <template x-for="so in filteredSalesOrders" :key="so.id">
                                                <option :value="so.id"
                                                    x-text="`${so.nomor} - ${formatCurrency(so.total)}`"
                                                    :selected="so.id ==
                                                        {{ old('sales_order_id', $uangMuka->sales_order_id ?? 'null') }}">
                                                </option>
                                            </template>
                                        </select>
                                    ...
                                    <div class="md:col-span-2">
                                        ...
                                        <div class="mt-1 relative rounded-md shadow-sm">
                                            ...
                                            <input type="number" name="jumlah" id="jumlah" x-model="jumlah"
                                                value="{{ old('jumlah', $uangMuka->jumlah) }}" min="0"
                                                step="0.01"
                                    ...
                                    <div class="md:col-span-2">
                                        ...
                                        <textarea name="keterangan" id="keterangan" rows="3"
                                            class="mt-1 block w-full border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-white"
                                            placeholder="Masukkan keterangan...">{{ old('keterangan', $uangMuka->keterangan) }}</textarea>
                                        @error('keterangan')
                                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                        @enderror
                                    </div>
                            ...
                            <div class="p-6">
                                    ...
                                    <div class="grid grid-cols-2 gap-4">
                                        ...
                                        <label class="relative">
                                            <input type="radio" name="metode_pembayaran" value="kas"
                                                ...
                                                {{ old('metode_pembayaran', $uangMuka->metode_pembayaran) === 'kas' ? 'checked' : '' }}>
                                        ...
                                        <label class="relative">
                                            <input type="radio" name="metode_pembayaran" value="bank"
                                                x-model="metode" class="sr-only"
                                                {{ old('metode_pembayaran', $uangMuka->metode_pembayaran) === 'bank' ? 'checked' : '' }}>
                                            <div class="p-4 border-2 rounded-lg cursor-pointer transition-all"
                                                :class="metode === 'bank' ?
                                                    'border-blue-500 bg-blue-50 dark:bg-blue-900/20' :
                                                    'border-gray-200 dark:border-gray-600'">
                                ...
                                <div id="kas_field" style="display: none;" class="mb-4">
                                    ...
                                    <select name="kas_id" id="kas_id"
                                        ...
                                        @foreach ($kasAccounts as $kas)
                                            <option value="{{ $kas->id }}"
                                                {{ old('kas_id', $uangMuka->kas_id) == $kas->id ? 'selected' : '' }}>
                                                {{ $kas->nama }} - Saldo: Rp
                                    ...
                                    <select name="rekening_bank_id" id="rekening_bank_id"
                                        ...
                                        @foreach ($bankAccounts as $bank)
                                            <option value="{{ $bank->id }}"
                                                {{ old('rekening_bank_id', $uangMuka->rekening_bank_id) == $bank->id ? 'selected' : '' }}>
                                ...
                                <div>
                                    ...
                                    <input type="text" name="nomor_referensi" id="nomor_referensi"
                                        value="{{ old('nomor_referensi', $uangMuka->nomor_referensi) }}"
                                        class="mt-1 block w-full border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-white"
                            ...
                            <div class="p-6 border-t border-gray-200 dark:border-gray-700">
                                <div class="flex justify-end space-x-3">
                                    ...
                                    <button type="button" @click="submitForm()"
                                        class="inline-flex items-center px-6 py-2 bg-amber-600 hover:bg-amber-700 text-white font-medium rounded-lg shadow-sm transition-colors duration-200"
                                        ...
                                        <span x-show="!isSubmitting">Update Uang Muka</span>
                                        <span x-show="isSubmitting" x-cloak>Mengupdate...</span>
                                    </button>
                                </div>
                            </div>
                        </div>
                                ...
                                <div class="space-y-4">
                                    ...
                                    <div
                                        ...
                                        <div class="font-semibold text-green-900 dark:text-green-100 text-sm">
                                            <span x-show="!metode"
                                                class="text-gray-500 dark:text-gray-400">{{ $uangMuka->metode_pembayaran === 'kas' ? 'Pembayaran Kas' : 'Transfer Bank' }}</span>
                                    ...
                                    <!-- Sales Order -->
                                    <div x-show="selectedSalesOrder || {{ $uangMuka->sales_order_id ? 'true' : 'false' }}"
                                        ...
                                        <div class="font-semibold text-yellow-900 dark:text-yellow-100 text-sm">
                                            ...
                                            @if ($uangMuka->salesOrder && !$uangMuka->sales_order_id)
                                                <span>{{ $uangMuka->salesOrder->nomor }} -
                                                    {{ 'Rp ' . number_format($uangMuka->salesOrder->total, 0, ',', '.') }}</span>
                                            @endif
                                        </div>
                                    </div>
                                    ...
                                    <!-- Status -->
                                    <div
                                        class="p-4 bg-gradient-to-r from-purple-50 to-purple-100 dark:from-purple-900/20 dark:to-purple-800/30 rounded-lg border border-purple-200 dark:border-purple-700">
                                        <div class="flex items-center mb-2">
                                            <svg class="w-4 h-4 text-purple-500 dark:text-purple-400 mr-2"
                                                fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        ...
                                        <div class="font-semibold text-purple-900 dark:text-purple-100 text-sm">
                                ...
                                <div
                                    ...
                                    <ul class="text-xs text-indigo-700 dark:text-indigo-300 space-y-1.5">
                                        ...
                                        <li class="flex items-start">
                                            ...
                                            Terakhir diubah: {{ $uangMuka->updated_at->format('d/m/Y H:i') }}
                                        </li>
                                        @if ($uangMuka->aplikasi()->exists())
                                            <li class="flex items-start">
                                                ...
                                                Aplikasi ke {{ $uangMuka->aplikasi->count() }} invoice
                                            </li>
                                        @endif
                                    </ul>
                                </div>
    ...
    @push('scripts')
        ...
        <script>
            function uangMukaEditForm() {
                return {
                    metode: @json(old('metode_pembayaran', $uangMuka->metode_pembayaran)),
                    selectedCustomer: @json(old('customer_id', $uangMuka->customer_id)),
                    selectedSalesOrder: @json(old('sales_order_id', $uangMuka->sales_order_id)),
                    jumlah: @json(old('jumlah', $uangMuka->jumlah)),
                    isSubmitting: false,
                    filteredSalesOrders: [],
