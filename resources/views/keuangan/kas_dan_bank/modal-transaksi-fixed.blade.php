    ...
    window.modalTransaksiManager = function() {
        return {
            ...
            async loadAvailableAccounts() {
                if (!this.formData.account_type) return;
            ...
            get filteredContraAccounts() {
                if (!this.formData.jenis) return this.contraAccounts;

                return this.contraAccounts.filter(account => {
                    const accountType = account.tipe?.toLowerCase();

                    if (this.formData.jenis === 'masuk') {
                        // For income: show income, revenue, liability, equity accounts
                        return ['pendapatan', 'revenue', 'income', 'liability', 'kewajiban', 'equity',
                            'modal'
                        ].some(type =>
                            accountType?.includes(type)
                        );
                    } else if (this.formData.jenis === 'keluar') {
                        // For expense: show expense, cost, asset accounts
                        return ['beban', 'expense', 'cost', 'biaya', 'asset', 'aktiva'].some(type =>
                            accountType?.includes(type)
                        );
                    }
            ...
            updateJournalPreview() {
                ...
                if (this.formData.jenis === 'masuk') {
                    ...
                    this.journalPreview.push({
                        account: `${kasAccount.nama}${kasAccount.nomor_rekening ? ' - ' + kasAccount.nomor_rekening : ''}`,
                        debit: amount,
                        kredit: 0
                    });
                    ...
                    <div class="grid grid-cols-1 gap-6">
                        ...
                        <div>
                            ...
                            <select name="contra_account_id" id="contra_account_id"
                                x-model="formData.contra_account_id" required
                                class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-primary-500 focus:ring-primary-500 text-sm">
