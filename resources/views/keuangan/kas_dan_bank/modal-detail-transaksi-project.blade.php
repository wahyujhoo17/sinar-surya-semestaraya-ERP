...
"
    @close-detail-transaksi-project.window="isOpen = false" @keydown.escape.window="isOpen = false" x-show="isOpen" x-cloak
    ...
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div x-show="isOpen" x-transition:enter="ease-out duration-300"
            ...
            <div x-show="showPrintNotification" x-transition:enter="transition ease-out duration-300"
                x-transition:enter-start="opacity-0 transform translate-y-2"
                x-transition:enter-end="opacity-100 transform translate-y-0"
                x-transition:leave="transition ease-in duration-200"
                x-transition:leave-start="opacity-100 transform translate-y-0"
                x-transition:leave-end="opacity-0 transform translate-y-2"
                class="mt-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-md p-4">
                <div class="flex">
                    ...
                    <div class="ml-3">
                        <p class="text-sm text-blue-800 dark:text-blue-200">
                            <strong>PDF sedang disiapkan!</strong> File akan terbuka di tab baru dalam beberapa detik.
                        </p>
                    </div>
                </div>
            </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6" x-show="project">
                            ...
                            <div class="flex items-center">
                                ...
                                <div class="ml-4">
                                    ...
                                    <dd class="text-lg font-semibold text-blue-900 dark:text-blue-100"
                                        x-text="formatRupiah(project?.total_alokasi || 0)"></dd>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="bg-red-50 dark:bg-red-900/20 rounded-lg p-4">
                            <div class="flex items-center">
                                ...
                                <div class="ml-4">
                                    <dt class="text-sm font-medium text-red-600 dark:text-red-400">Total Penggunaan
                                    </dt>
                                    <dd class="text-lg font-semibold text-red-900 dark:text-red-100"
                                        x-text="formatRupiah(project?.total_penggunaan || 0)"></dd>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="bg-green-50 dark:bg-green-900/20 rounded-lg p-4">
                            <div class="flex items-center">
                                ...
                                <div class="ml-4">
                                    <dt class="text-sm font-medium text-green-600 dark:text-green-400">Saldo Tersisa
                                    </dt>
                                    <dd class="text-lg font-semibold text-green-900 dark:text-green-100"
                                        x-text="formatRupiah(project?.saldo || 0)"></dd>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="overflow-hidden shadow ring-1 ring-black ring-opacity-5 md:rounded-lg">
                                ...
                                <template x-for="trx in transaksi" :key="trx.id">
                                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100"
                                            x-text="formatDate(trx.tanggal)"></td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <span
                                                class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium capitalize"
                                                :class="getJenisColor(trx.jenis)">
                                                <svg class="w-3 h-3 mr-1" fill="none" stroke="currentColor"
                                                    viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round"
                                                        stroke-width="2" :d="getJenisIcon(trx.jenis)"></path>
                                                </svg>
                                                <span x-text="trx.jenis"></span>
                                            </span>
                                        </td>
                                        <td class="px-6 py-4 text-sm text-gray-900 dark:text-gray-100"
                                            x-text="trx.keterangan"></td>
                                        <td
                                            class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                                            <span x-show="trx.kas" x-text="trx.kas?.nama"></span>
                                            <span x-show="trx.rekening_bank"
                                                x-text="trx.rekening_bank?.nama_bank + ' - ' + trx.rekening_bank?.nomor_rekening"></span>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <span
                                                :class="trx.jenis === 'penggunaan' ? 'text-red-600 dark:text-red-400' :
                                                    'text-green-600 dark:text-green-400'"
                                                x-text="formatRupiah(trx.nominal)"></span>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400"
                                            x-text="trx.user?.name"></td>
                                    </tr>
                                </template>
                            </tbody>
                        </table>
                        ...
                        <div x-show="transaksi.length === 0 && !isLoading" class="text-center py-12">
                            <svg class="mx-auto h-12 w-12 text-gray-400 dark:text-gray-600" fill="none"
                                ...
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M20 13V6a2 2 0 00-2-2H6a2 2 0 00-2 2v7m16 0v5a2 2 0 01-2 2H6a2 2 0 01-2-2v-5m16 0h-6m-10 0H4m6 0v4m4-4v4m-4-4a2 2 0 012-2h0a2 2 0 012 2m-6 0a2 2 0 012-2h0a2 2 0 012 2">
                                </path>
                            </svg>
                            <h3 class="mt-2 text-sm font-medium text-gray-900 dark:text-gray-100">Belum ada transaksi
                            </h3>
                            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Project ini belum memiliki
                        ...
                        <div class="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
                            <div>
                                <p class="text-sm text-gray-700 dark:text-gray-300">
                                    Showing page <span class="font-medium" x-text="currentPage"></span> of <span
                            ...
                            <div>
                                <nav class="isolate inline-flex -space-x-px rounded-md shadow-sm"
                                    ...
                                    <button
                                        @click="currentPage < totalPages && loadTransaksi(project.id, currentPage + 1)"
                                        :disabled="currentPage >= totalPages"
            ...
            <div class="mt-6 flex justify-between items-center border-t border-gray-200 dark:border-gray-700 pt-4">
                <div class="flex space-x-2">
                    <button type="button"
                        @click="project && (showPrintNotification = true, setTimeout(() => showPrintNotification = false, 2000), window.open(`{{ route('keuangan.transaksi-project.pdf', ':id') }}`.replace(':id', project.id), '_blank'))"
                        x-show="project"
                        class="inline-flex items-center px-4 py-2 bg-primary-600 border border-transparent rounded-md shadow-sm text-sm font-medium text-white hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 transition-colors duration-200">
                        <svg class="w-4 h-4 mr-2" xmlns="http://www.w3.org/2000/svg" fill="none"
                            viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round"
                                d="M6.72 13.829c-.24.03-.48.062-.72.096m.72-.096a42.415 42.415 0 0110.56 0m-10.56 0L6.34 18m10.94-4.171c.24.03.48.062.72.096m-.72-.096L17.66 18m0 0l.229 2.523a1.125 1.125 0 01-1.12 1.227H7.231c-.662 0-1.18-.568-1.12-1.227L6.34 18m11.32 0H6.34m11.32 0l1.836-1.836a.75.75 0 000-1.061l-1.836-1.836a.75.75 0 00-1.061 0L14.757 15.1a.75.75 0 000 1.061l1.836 1.836zm-11.32 0l-1.836-1.836a.75.75 0 010-1.061l1.836-1.836a.75.75 0 011.061 0L7.243 15.1a.75.75 0 010 1.061L5.407 16.96z" />
                        </svg>
                        Cetak PDF
                    </button>
                    <button type="button"
                        @click="project && navigator.clipboard.writeText(`Project: ${project.nama}\nKode: ${project.kode}\nBudget: ${formatRupiah(project.budget)}\nTotal Alokasi: ${formatRupiah(project.total_alokasi)}\nTotal Penggunaan: ${formatRupiah(project.total_penggunaan)}\nSaldo: ${formatRupiah(project.saldo)}`).then(() => alert('Data project berhasil disalin ke clipboard!'))"
                        x-show="project"
                        class="inline-flex items-center px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 transition-colors duration-200">
                        <svg class="w-4 h-4 mr-2" xmlns="http://www.w3.org/2000/svg" fill="none"
                            viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round"
                ...
                <button type="button" @click="isOpen = false"
                    class="inline-flex items-center px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 transition-colors duration-200">
                    Tutup
                </button>
            </div>
        </div>
