                ...
                <div class="grid grid-cols-1 lg:grid-cols-4 gap-4">
                    ...
                    <div>
                        <label for="role-filter"
                            class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Filter Role:</label>
        ...
        function managementPengguna() {
            return {
                ...
                init() {
                    ...
                    this.roles = @json(
                        $roles->map(function ($role) {
                            return ['id' => $role->id, 'name' => $role->nama ?? $role->name];
                        }));
                ...
                async resetUserPassword(userId) {
                    ...
                    try {
                        const response = await fetch(
                            `{{ route('pengaturan.management-pengguna.index') }}/${userId}/reset-password`, {
                                method: 'PATCH',
                                headers: {
                                    'X-Requested-With': 'XMLHttpRequest',
                                    'X-CSRF-TOKEN': '{{ csrf_token() }}'
