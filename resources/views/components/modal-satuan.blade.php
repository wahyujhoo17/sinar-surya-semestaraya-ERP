<div x-data="modalSatuanManager()" x-show="isOpen" @open-satuan-modal.window="openModal($event.detail)"
    @close-satuan-modal.window="closeModal()" x-cloak class="fixed inset-0 z-50 overflow-y-auto">
    <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div class="inline-block align-bottom bg-white dark:bg-gray-800 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-md sm:w-full"
            ...
            <div class="px-6 py-4">
                <form id="satuan-form" @submit.prevent="submitForm()">
                    @csrf
...
<script>
    function modalSatuanManager() {
        return {
            isOpen: false,
            isEdit: false,
            modalTitle: 'Tambah Satuan',
            loading: false,
            satuanId: null,
            formData: {
                kode: '',
            ...
            openModal(data = {}) {
                this.resetForm();
                if (data.mode === 'edit' && data.satuan) {
                    this.isEdit = true;
                    this.modalTitle = 'Edit Satuan';
                    this.satuanId = data.satuan.id;
                    this.formData.kode = data.satuan.kode;
                    this.formData.nama = data.satuan.nama;
                    this.formData.deskripsi = data.satuan.deskripsi;
                } else {
                    this.isEdit = false;
                    this.modalTitle = 'Tambah Satuan';
                }
                this.isOpen = true;
            },
            closeModal() {
            ...
            submitForm() {
                ...
                let url, method;
                if (this.isEdit) {
                    url = `/master-data/satuan/${this.satuanId}`;
                    method = 'POST';
                ...
                fetch(url, {
                    ...
                    .then(data => {
                        ...
                        window.dispatchEvent(new CustomEvent('refresh-satuan-table'));
                        this.closeModal();
                    })
                    .catch(error => {
                        this.loading = false;
