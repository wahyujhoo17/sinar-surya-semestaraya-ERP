...
<div x-data="{
    ...
    showConfirm: true,
    showCancel: false,
    // Flag to prevent multiple opens
    processing: false
}"
    @alert-modal.window="
    ...
    "
    id="{{ $id }}" class="fixed z-50 inset-0 overflow-y-auto" aria-labelledby="modal-title" role="dialog"
    aria-modal="true" x-show="open" x-cloak>
    <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        <div x-show="open" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0"
            x-transition:enter-end="opacity-100" x-transition:leave="ease-in duration-200"
            x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0"
            class="fixed inset-0 bg-gray-500 dark:bg-gray-900 bg-opacity-75 dark:bg-opacity-75 transition-opacity"
            @click="if(!$event.target.closest('.modal-content')) { open = false; onCancel(); }" aria-hidden="true"
            x-cloak></div>
        ...
        <div x-show="open" x-cloak x-transition:enter="ease-out duration-300"
                ...
                <div class="sm:flex sm:items-start">
                    <div class="mx-auto flex-shrink-0 flex items-center justify-center h-12 w-12 rounded-full sm:mx-0 sm:h-10 sm:w-10"
                        ...
                        <template x-if="type === 'warning'">
                            <svg class="h-6 w-6 text-yellow-600 dark:text-yellow-400" xmlns="http://www.w3.org/2000/svg"
                                fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round"
                                    d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.007v.008H12v-.008z" />
                            </svg>
            ...
            <div
                class="bg-gray-50 dark:bg-gray-800 border-t dark:border-gray-700 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                <template x-if="showConfirm">
                    <button
                        ...
                        processing = true;
                        open = false; 
                        // Delay function execution to ensure modal closes first
                        setTimeout(() => {
                            onConfirm();
                            processing = false;
                        }, 200);"
